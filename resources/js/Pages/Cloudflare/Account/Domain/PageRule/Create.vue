<template>
    <div class="bg-white p-4 rounded min-h-screen shadow-md">
        <h3 class="text-3xl mb-6">Create Page Rule for "{{domain.name}}"</h3>
...
<!--            <div v-if="error.errors.account_id" class="text-red-600 text-xs mx-1 mt-1">{{ error.errors.account_id[0] }}</div>-->
        ...
        <div class="mb-3">
            <button @click.prevent="storePageRule" class="p-3 text-white rounded text-center hover:bg-emerald-500 bg-emerald-400">Create</button>
        </div>
        <div>
...
export default {
    ...
    methods: {
        storePageRule() {
            this.error = {}
            axios.post(`/cloudflare-accounts/${this.cloudflareAccount.id}/domains/${this.domain.id}/page-rules`, this.pageRule)
                .then(res => {
